...
const resolveTexturePath = (name) => {
  ...
  if (hasExt) {
    return encodeURI(`/textures/${name}`);
  }

  if (name === "book-cover" || name === "book-back") {
    return encodeURI(`/textures/${name}.jpg`);
  }

  return encodeURI(`/textures/${name}.webp`);
};
...
const Page = ({ number, front, back, page, opened, bookClosed, ...props }) => {
  ...
  const [picture, picture2, pictureRoughness] = useTexture(texturePaths);
  ...
  const manualSkinnedMesh = useMemo(() => {
    ...
    const materials = [
      ...
      new MeshStandardMaterial({
        color: whiteColor,
        map: picture,
        ...(number === 0 ? { roughnessMap: pictureRoughness } : { roughness: 0.4 }),
